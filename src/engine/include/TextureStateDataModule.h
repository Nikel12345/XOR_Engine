...
//
// Механизм — тот же, что у разреженного текст-канала UI (UI_DataModule: bits/wordbase/index/text),
// шейдерные примитивы у них общие (shaders_code/sparse_rank.hlsli). Переключает варианты малая
// доля сущностей, поэтому полноразмерным остаётся только presence-бит, а всё остальное живёт по
// числу НОСИТЕЛЕЙ. Прежняя раскладка (int-смещение на строку) тратила 32 бита на строку ради
// значения, которое почти везде значит «ничего нет»: на 800k это 3.05 МиБ против 200 КБ теперь.
//
// bits и wordbase сведены в ОДИН буфер (uint2), в отличие от текст-канала, где это два. Причина
// шейдерная: пара лежит в одной кэш-линии и читается одной загрузкой, а в общем прологе материалов
// на один бинд меньше — каждый буфер там обязана биндить КАЖДАЯ sp, включившая пролог.
//
// ОДИН модуль на три буфера (ограничение «инструкция пишет один буфер» реально, «модуль
// обслуживает один буфер» нет: LightDataModule держит два, UI_DataModule четыре). Держать их
...
private:
	uint32_t RankWords() const { return (rows_ + 31u) / 32u; }
