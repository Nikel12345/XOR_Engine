...
// Нагрузка команд, привязанных к ПОЛЮ схемы (FieldSpec::cmd). Продьюсер один — generic-редактор
// компонентов (ui::DrawComponentFields): он знает ровно «какая энтити, какое поле какого
// компонента и новое значение», и этого достаточно любой такой команде. Хендлер берёт нужное
// (visible — num, имя ассета — str), делает движковую работу мимо ECS (дельта в батчи, резолв)
// и сам пишет значение — UI-поток колонку НЕ трогает.
struct FieldEditCmd {
    uint32_t    entity;      // Entity как uint32_t — заголовок не завязан на BaseComponents.h
    std::string component;   // имя компонента в реестре: "Draw", "Model", ...
    std::string field;       // ключ поля: "visible", "name", ...
    double      num = 0.0;   // числовые виды (F32/U32/Bool/Angle)
    std::string str;         // строковые (Str/Asset*)
};
...
// текстуру по id ячейки → сами перепривяжутся на пересборке. Строки на куче, функтор удалит.
struct UpsertTextureCmd {
    ...
    // Грузить файл как кубмапу-крест 4×3 (EngineContext::CreateCubeMapTexture) вместо обычной
    // текстуры. Приходит из формы, а не выводится из типа атласа: атлас в форме выбирает тот же
    // человек, и молча менять смысл его выбора команда не должна — несовпадение поймает и назовёт
    // проверка в CreateCubeMapTexture. conv к кубу не применяется.
    bool        cube = false;
};

// Удаление текстуры. Материалы, ссылавшиеся на её ячейку, на пересборке дадут dummy.
...
// Создание сущности из формы (UI_Hierarchy): staging-энтити сериализована в json
// (ObjectManager::SaveScene staging-сцены — одна сущность). Хендлер в sim-потоке гонит её
// ТЕМ ЖЕ путём, что загрузку сцены: LoadScene(scene, json) → фиксап model/material указателей
// по именам → пересборка батчей. Формат/дефолты/ремапы формы и файла едины по построению.
struct CreateEntityCmd {
    std::string scene;   // целевая сцена (активная на момент нажатия Create)
    std::string json;    // текст сцены из одной сущности
};
