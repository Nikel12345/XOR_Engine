...
namespace DefaultRenderPassNamespace
{
    ...
    // Частота сэмплирования СЦЕНЫ относительно окна: 2.0 = SSAA 2x (рисуем вчетверо больше пикселей,
    // present-блит усредняет их вниз), меньше единицы — масштабирование ради fps. Множителем, а не
    // абсолютом, чтобы настройка пережила ресайз окна. Менять можно в любой момент: сценовые таргеты
    // пересоздаст ближайший кадр, а игра узнаёт разрешение не отсюда, а с самого таргета.
    void  SetSceneResolutionScale(float scale);
    ...
    struct alignas(16) BloomState {
        ...
        // Доля ОКНА у уровня 0 пирамиды (дальше /2 на уровень). База у всех экранных таргетов одна —
        // окно; частота сэмплирования сцены сюда не входит, иначе это поле значило бы разное при
        // разных её значениях. Живёт ЗДЕСЬ, а не в общей настройке набора: проходы — открытое
        // множество, и доля есть только у вошедшего в него эффекта.
    ...
    struct alignas(16) AOState {
        ...
        float bias      = 0.01f;
        // Доля ОКНА у карты AO — там же, где у блума, и по той же причине. Ограничения
