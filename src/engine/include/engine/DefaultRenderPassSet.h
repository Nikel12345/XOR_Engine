...
namespace DefaultRenderPassNamespace
{
    ...
    // Частота сэмплирования СЦЕНЫ относительно базы (сегодня база = окно): 2.0 = SSAA 2x (рисуем
    // вчетверо больше пикселей, present-блит усредняет их вниз), меньше единицы — масштабирование
    // ради fps. Множителем, а не абсолютом, чтобы настройка пережила ресайз окна. Менять можно в
    // любой момент: сценовые таргеты пересоздаст ближайший кадр, а игра узнаёт разрешение не отсюда,
    // а с самого таргета.
    // ТОЛЬКО с render-потока — почему именно так, см. определение в DefaultRenderPassSet.cpp.
    void  SetSceneResolutionScale(float scale);
    float SceneResolutionScale();

    // PassSystem: общие ресурсы дефолтного набора проходов — ВСЕ экранные таргеты и их инструкции
    // ресайза. Должна вызываться ПЕРЕД Set*Pass, которые их потребляют (main/transparent/debug/ao/bloom).
    // (out_w,out_h) — размер окна: от него все они и считаются.
