...
namespace DefaultShaderProgramSet
{
    ...
    // Код-байндинги движковых RENDER-программ: типы типовых пушей (реестр ShaderManager) и
    // именные пуши тех программ, у которых свой блок. Зовёт движок ПЕРЕД созданием шейдеров:
    // разбор маркеров //@push сверяется с реестром типов прямо на компиляции, и тип,
    // зарегистрированный позже, будет отмечен в логе как неизвестный.
