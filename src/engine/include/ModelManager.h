...
struct SceneModelEntry {
	...
	// Диапазоны сабмешей, по одному на сабмеш и в порядке сабмешей модели (ключ
	// "screen_size_span" манифеста). Короче/длиннее фактического числа сабмешей — законно:
	// лишнее игнорируется, недостающим остаётся (0,0) = «границ нет».
...
public:
	...
	// Зовётся раз в кадр в начале PrepareFunc — то есть ДО того, как PackModels разместит пачку
	// этого кадра; только так перезагруженная модель садится в освободившееся от неё же место в
	// ТОМ ЖЕ кадре (сначала освобождение, потом размещение).
	//
	...
	// РАЗМЕЩЕНИЕ пачки кадра: выбрать ей место в буферах пулов и перевести смещения сабмешей из
	// стейджинг-относительных в абсолютные. Зовётся в PrepareFunc рядом с TextureManager::PackAtlases
	// и ровно по той же причине: батч копирует размещение ЗНАЧЕНИЯМИ (ModelBatchData::submesh), а
	// значит оно обязано быть окончательным ДО сборки дерева. Байты пишут инструкции заливки, они
	// размещения больше не касаются. Идемпотентна: повторный вызов до заливки ничего не сдвинет.
	void PackModels();
	...
	// Запись диапазона + бамп ревизии ОДНОЙ операцией: разъехаться им нельзя, забытый бамп даёт
	// правку, которая молча не доедет до GPU. Промах имени/индекса и запись того же значения —
	// no-op (редактор дёргает это на каждый кадр перетаскивания ползунка).
...
private:
	...
	struct PoolResidency {
		...
		bool batch_placed = false;   // смещения пачки уже переведены в абсолютные (см. PackModels)
