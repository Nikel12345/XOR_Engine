...
public:
	...
	// Монотонная ревизия ДИАПАЗОНОВ сабмешей (SubMeshData::screen_size_span). Отдельная от
	// spheres_revision, потому что гейтит другой буфер: спаны едут в EntityToCmd, сферы — в
	// BoundSpheres, и общая ревизия перезаливала бы оба на любую правку любого.
	//
	// Нужна именно ревизия, а не «просто записать поле»: содержимое EntityToCmd — функция ДВУХ
	// вещей, дерева батчей и спанов, а гейт у него был только по первому. Правка спана дерево не
	// трогает, поэтому без этого счётчика новое значение лежало бы в памяти и ждало ближайшей
	// структурной пересборки. Читатель складывает обе ревизии — обе монотонны, значит сумма
	// меняется на изменение любой (тот же приём, что у BoundSpheres: EntityRevision + Spheres).
	uint64_t SpansRevision() const { return spans_revision; };
...
private:
	...
	uint64_t spans_revision   = 0;   // см. SpansRevision()
};
