...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    // Шаг ядра — в текселях ПРИЁМНИКА, а не источника: внешнее кольцо (±2t) обязано лечь ровно на
    // ±1 тексель приёмника, иначе 13 тапов накроют не тот участок, что сваливается в один тексель.
    // При сокращении 2:1 это ровно прежнее 1/sw (0.5/dw = 1/sw), но отношение здесь больше не
    // обязано равняться двум: источник — scene_hdr в render-разрешении, приёмник — уровень 0 в
    // эффектном (частота сэмплирования сцены против BloomState::resolution_scale). Со старым шагом при
    // render выше окна ядро читало бы четверть своего футпринта, то есть недосэмплило бы ровно ту
    // яркую мелочь, ради которой суперсэмплинг и включён.
    float2 t  = 0.5 / float2(dw, dh);
