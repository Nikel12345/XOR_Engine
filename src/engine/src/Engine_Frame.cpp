...
void Engine::PrepareFunc(uint8_t slot)
{
	...
	{
		PROF_SCOPE(Sim, " pack_models");
		model_manager->PackModels();
	}
...
void Engine::UploadFunc(uint8_t slot)
{
	...
	transfer_manager->MarkReturnable(pending_upload_tbs[slot]);
	pending_upload_tbs[slot] = nullptr;
	transfer_manager->MarkReturnable(pending_texture_tbs[slot]);
...
bool Engine::RenderFunc(uint8_t slot)
{
	...
	// Размер назначения берём из свопчейна: в него и бьёт present-блит. Инструкции зовутся безусловно
	// каждый кадр — пересоздание гейтит сам таргет, сравнивая с применённым
	// (TextureManager::RecreateAtlasTexture).
