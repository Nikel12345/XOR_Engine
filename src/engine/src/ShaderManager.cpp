...
ComputeShaderProgram* ShaderManager::CreateComputeShaderProgram(const std::string& name, const std::string& cs_name,
    ...
    BufferManager* bm, TextureManager* tm, ResourceTag tags)
{
...
bool ShaderManager::DeleteComputeShader(ComputeShaderId id, NameSlot slot)
{
    ...
    return Release(compute_shaders, id, slot);
}
