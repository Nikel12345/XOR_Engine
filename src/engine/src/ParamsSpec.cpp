...
ParamsSpecRegistry& ParamsSpecRegistry::Passes()
{
    static ParamsSpecRegistry instance;
    return instance;
}

void ParamsSpecRegistry::Register(ParamsSpec s)
{
    if (s.name.empty()) {
    ...
    for (size_t i = 0; i < s.fields.size(); ) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
            "ParamsSpec '%s': field '%s' (offset %u, %u bytes) does not fit into %zu-byte blob - dropped",
...
const ParamsSpec* ParamsSpecRegistry::ByName(const std::string& name) const
{
    auto it = by_name_.find(name);
    return it == by_name_.end() ? nullptr : &specs_[it->second];
}

const ParamsSpec* ParamsSpecRegistry::ByType(std::type_index t) const
{
...
const std::string& MaterialParamsTypeName(std::type_index t)
{
    ...
    if (const ParamsSpec* s = ParamsSpecRegistry::Materials().ByType(t)) return s->name;
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
        "SetMaterialParams: type '%s' is not registered (ParamsSpecRegistry::Register) - "
...
void RegisterBuiltinMaterialParamsSpecs()
{
    ...
    reg.Register(MakeParamsSpec<OpaqueMaterialParams>("Opaque", {
        ...
        ParamsFieldSpec::Num(PARAMS_FIELD(OpaqueMaterialParams, emissiveStrength), K::F32, 0, 8).Label("Emissive Strength"),
        ParamsFieldSpec::Num(PARAMS_FIELD(OpaqueMaterialParams, metallic),         K::F32, 0, 1).Label("Metallic"),
        ParamsFieldSpec::Num(PARAMS_FIELD(OpaqueMaterialParams, roughness),        K::F32, 0, 1).Label("Roughness"),
        // heightScale: 0 = POM выключен; рабочая зона ≤0.05 (глубже — «плавание» на близи).
        ParamsFieldSpec::Num(PARAMS_FIELD(OpaqueMaterialParams, heightScale),      K::F32, 0, 0.2f, 0.001f).Label("POM Height"),
    ...
    reg.Register(MakeParamsSpec<UIMaterialParams>("UI", {
        ...
        ParamsFieldSpec::Num(PARAMS_FIELD(UIMaterialParams, text_height), K::F32, 0, 1).Label("Text Height"),
        ParamsFieldSpec::Num(PARAMS_FIELD(UIMaterialParams, text_anchor), K::F32, 0, 1).Label("Text Anchor (0=top,1=bot)"),
    }));
}
