...
namespace {
    // shadow_pib = сумма инстансов теневых команд = ГРАНИЦА PIB между теневыми записями [0, shadow_pib)
    // и остальными [shadow_pib, N). Она же first_instance первой не-теневой команды (FinalizeOffsets
    // нумерует по проходам, shadow первый по pass_index=10). Так каждая scatter-программа берёт свой
    // диапазон PIB. Идёт под BatchTreeMutex (вызов из push во время ExecutePrepassesSteps).
