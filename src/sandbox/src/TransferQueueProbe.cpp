...
//    3. валидация Vulkan молчит ВО ВРЕМЯ РАБОТЫ;
...
//
//  ОЖИДАЕМЫЙ ШУМ ВАЛИДАЦИИ НА ВЫХОДЕ. При разрушении устройства прилетает
//  несколько VUID-vkDestroyDevice-device-05137 про живые VkShaderModule. Это НЕ
//  про очереди и не дефект зонда: шейдеры, которых не держит ни одна
//  ShaderProgram, лежат в именованных реестрах ShaderManager, а те разрушаются
//  ПОСЛЕ гашения токена менеджера — делитер тогда сознательно становится no-op,
//  чтобы не звать релиз по мёртвому устройству (см. комментарий у
//  ShaderManager::RegisterGpuShader). Ресурсы добивает само уничтожение
//  устройства. Значимо только то, что во время фаз 1-3 валидация молчит.
//
...
int main(int, char**)
{
    ...
    {   // менеджеры — во вложенной области: их деструкторы освобождают GPU-ресурсы и обязаны
        ...
        *pipeline_slot = pipes.GetGraphicPipeline(sp);
        ...
        SDL_Log("--- фаза 3: отрисовка (render queue) ---");
        const int FRAMES = 90;
        for (int i = 0; i < FRAMES; ++i) {
            SDL_Event ev;
            while (SDL_PollEvent(&ev)) { if (ev.type == SDL_EVENT_QUIT) i = FRAMES; }
        ...
        SDL_Log("Кадров с отрисованным треугольником: %u из %d", g_frames_drawn, FRAMES);
