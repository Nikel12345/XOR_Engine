...
- **Заголовки разложены по ярусам** — `src/engine/include/{core,ecs,gpu,engine}/`, и каждая цель
  отдаёт наружу только свой каталог: нижние приходят по линковке. Поэтому включение вверх просто
  не резолвится, и границу слоёв держит компилятор. Сами `.cpp` лежат плоско в `src/` — ярус
  файла задаёт список его цели.
