...
попадает в ту же ячейку, и ссылающиеся продолжают показывать в неё. Так устроены upsert редактора и
перезагрузка сцены. **Удаление** (`NameSlot::Release`) отпускает и имя: ячейка становится
безымянной, а то же имя, заведённое позже, получает новую. Пока ячейка пуста, реестр отдаёт
